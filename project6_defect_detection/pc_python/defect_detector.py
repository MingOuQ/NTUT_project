...
# 儲存路徑（使用絕對路徑，確保不受工作目錄影響）
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SAVE_DIR = os.path.join(_SCRIPT_DIR, "captured_images")
EXCEL_DIR = os.path.join(_SCRIPT_DIR, "inspection_reports")
...
class DefectDetectionSystem:
    ...
    def run_inspection(self):
        ...
        # 匯出瑕疵座標至 Excel
        if all_defects:
            self.export_defects_to_excel(all_defects)
    ...
    def export_defects_to_excel(self, defects):
        """
        將瑕疵座標與資訊匯出至 Excel 檔案

        每次檢測產生一個 .xlsx 檔，內含：
          - 瑕疵編號、中心座標 (X, Y)、寬高、類型、信心度
          - 若為 9panel 模式，包含面板編號與面板內座標
          - 檢測時間戳記
        """
        if not HAS_OPENPYXL:
            print("  [WARN] openpyxl not installed, skipping Excel export")
            print("  [WARN] Run: pip install openpyxl")
            return None

        print(f"\n  [EXCEL] Exporting {len(defects)} defect(s) to Excel...")
        print(f"  [EXCEL] Target folder: {EXCEL_DIR}")

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"defect_report_{timestamp}.xlsx"
        ...
        defect_fill = PatternFill(start_color="FCE4EC", end_color="FCE4EC", fill_type="solid")

        # ---- 標題區 ----
        ws.merge_cells("A1:H1")
        title_cell = ws["A1"]
        title_cell.value = "瑕疵檢測報告 - Surface Defect Inspection Report"
        title_cell.font = Font(name="Calibri", bold=True, size=14, color="1F4E79")
        title_cell.alignment = Alignment(horizontal="center", vertical="center")

        ws.merge_cells("A2:H2")
        info_cell = ws["A2"]
        info_cell.value = f"檢測時間: {datetime.now().strftime('%Y/%m/%d %H:%M:%S')}    模式: {self.mode}    瑕疵數量: {len(defects)}"
        info_cell.font = Font(name="Calibri", size=10, color="555555")
        info_cell.alignment = Alignment(horizontal="center")

        # ---- 表頭 ----
        if self.mode == "9panel":
            headers = ["#", "中心 X", "中心 Y", "寬度", "高度", "瑕疵類型", "信心度 (%)", "面板編號"]
        else:
            headers = ["#", "中心 X", "中心 Y", "寬度", "高度", "瑕疵類型", "信心度 (%)", "面積"]
        ...
        if self.mode == "9panel":
            img_for_grid = (
                self.images.get('red') or
                self.images.get('green') or
                self.images.get('blue')
            )
            if img_for_grid is not None:
                h, w = img_for_grid.shape[:2]
                cell_w = w // GRID_COLS
                cell_h = h // GRID_ROWS
                for i, d in enumerate(defects):
                    col = min(d['x'] // cell_w, GRID_COLS - 1)
                    row = min(d['y'] // cell_h, GRID_ROWS - 1)
                    panel_map[i] = row * GRID_COLS + col + 1
        ...
        for i, d in enumerate(defects):
            ...
            if self.mode == "9panel":
                last_col_val = panel_map.get(i, "-")
            else:
                last_col_val = d.get('area', d['w'] * d['h'])
        ...
        # ---- 欄寬自動調整 ----
        col_widths = [6, 10, 10, 10, 10, 14, 14, 12]
    ...
    def _capture_and_display(self, color):
        ...
        if img is not None:
            ...
            # 將檢測結果同步傳送至 ESP32S（以便即時廣播給 HTML 手機端與顯示在 OLED 螢幕）
            print(f"  [INFO] Reporting {len(defects)} defects in {color.upper()} to ESP32S...")
            self._send_results(defects)

            # 匯出瑕疵座標至 Excel
            if defects:
                self.export_defects_to_excel(defects)
                print(f"  [INFO] {len(defects)} defect(s) exported to Excel")
        self.ctrl.send("LED:OFF")
